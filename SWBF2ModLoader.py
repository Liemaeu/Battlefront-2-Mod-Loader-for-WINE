...
#Function to add the installed mods to the InstalledMods list
def addInstalledMods():
    for a in range(len(Mods)):
        modknown = False
        for i in range(len(ModList)):
            if Mods[a] == ModList[i][0]:
                InstalledMods.append(ModList[i][1])
                modknown = True
        if modknown == False:
            InstalledMods.append(Mods[a])
        else:
            modknown = False

addInstalledMods()
InstalledMods = sorted(InstalledMods)

#Function to check if the name of the mod is in the ModList
def getName(i):
    #checks if mod is unknown
    if len(InstalledMods[i]) < 4:
        return InstalledMods[i]
    else:
        for a in range(len(ModList)):
            if InstalledMods[i] == ModList[a][1]:
                return ModList[a][0]

#Function to enable a mod
def Enable(i):
    os.chdir(Directory + str(getName(i)))
...
for i in range(len(InstalledMods)): #Creates the window content
    tkinter.Label(frame, text = InstalledMods[i]).grid(row=i, sticky="nsew", column=0)
    tkinter.Label(frame, text = getName(i), fg="grey").grid(row=i, sticky="nsew", column=1)
